...
func (self *RedisCacheProvider) BuildList(group string, begin uint64, end uint64) error {
	...
	tempIdList := []string{}
	for i := begin; i < end; i++ {
		append(tempIdList, strconv.FormatUint(i, 10))
		last := bool(i == (end - 1))
		if ((i % 100) == 0) || last {
	...
	logging.LogInfof("The list of group '%s' is Builded. (begin=%d, end=%d)\n", begin, end)
	return nil
...
func (self *RedisCacheProvider) Pop(group string) (uint64, error) {
	...
	if len(value) == 0 {
		return -1, &error.EmptyListError{fmt.Sprintf("Empty List! (group=%s)", group)}
	}
